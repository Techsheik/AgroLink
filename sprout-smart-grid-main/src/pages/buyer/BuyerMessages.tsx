...
export default function BuyerMessages() {
      ...
      try {
        ...
        if (targetUserId) {
          ...
          if (existing) {
            ...
            // Create a temporary conversation object for the new recipient
            setSelected({
              other_user_id: parseInt(targetUserId),
              other_user_name: "Farmer", // Default until we get actual name or messages start
  ...
  // Polling for messages in selected conversation
  useEffect(() => {
    if (!selected) return;
  ...
  const statusBadge = (status: string) => {
    return <span className="text-[10px] font-medium px-1.5 py-0.5 rounded bg-emerald-100 text-emerald-700 uppercase tracking-wider">{status}</span>;
  };

  if (isLoading && conversations.length === 0) {
    return (
      <DashboardLayout navItems={buyerNavItems} title="Messages">
