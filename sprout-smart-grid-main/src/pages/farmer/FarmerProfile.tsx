...
export default function FarmerProfile() {
  ...
  if (isLoading) {
    return (
      <DashboardLayout navItems={farmerNavItems} title="Profile">
        ...
        <div className="max-w-2xl space-y-6">
          <div className="bg-card rounded-xl border p-6 flex items-center gap-5">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground capitalize">{user?.role} · {user?.is_verified ? "Verified" : "Pending Verification"}</p>
              <p className="text-xs text-muted-foreground mt-0.5">Account Active: {user?.is_active ? "Yes" : "No"}</p>
            </div>
          </div>
          ...
          <div className="bg-card rounded-xl border p-6 space-y-4">
            ...
            {[
              ...
              { label: "Verification Status", value: user?.is_verified ? "Verified" : "Unverified" },
            ].map((item) => (
