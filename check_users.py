...
try:
    ...
    with engine.connect() as conn:
        print("\n--- USERS ---")
        result = conn.execute(text("SELECT id, email, role, is_verified, is_active FROM users"))
        rows = result.fetchall()
        for row in rows:
            print(f"ID: {row.id}, Email: {row.email}, Role: {row.role}, Verified: {row.is_verified}, Active: {row.is_active}")
