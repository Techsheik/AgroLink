...
try:
    ...
    with engine.connect() as conn:
        print("\n--- CROPS TABLE SCHEMA ---")
        result = conn.execute(text("SELECT column_name, data_type FROM information_schema.columns WHERE table_name = 'crops'"))
        for row in result.fetchall():
            print(f"Column: {row.column_name}, Type: {row.data_type}")

        print("\n--- BUYER REQUESTS TABLE SCHEMA ---")
        result = conn.execute(text("SELECT column_name, data_type FROM information_schema.columns WHERE table_name = 'buyer_requests'"))
        for row in result.fetchall():
            print(f"Column: {row.column_name}, Type: {row.data_type}")
